...
//QUESTA VERSIONE PREVEDE SOLO DUE CLASSI
...
void itostr();   /* funzione che traduce un numero in una stringa
                    rappresentante il numero stesso */
...
void main(int argc,char **argv)
{
 ...
 for(K=Kmin;K<=Kmax;K+=2)  {   
    ...
    fprintf(fp,"%d\n",K);
    fclose(fp);
    stampaConfusione(fres,"NA",nclass,K);
/*        for (i=0;i<nclass;i++)
          {
