...
*                                                                           *
****************************************************************************/
//RISPETTO AI PROGRAMMI PRECEDENTI NORMALIZZA DIVIDENDO PER IL MAX - MIN
...
void calcolo(char *filename)
  {
   ...
   while(!feof(in))
    ...
    for(j=0; j<nparam; j++)
       ...
       if (feature[j] < min) min=feature[j];
       }

    for(k=0; k<nparam; k++)
        norm[k]=(feature[k]-min)/(max-min);
