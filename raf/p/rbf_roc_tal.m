...
trn=load('C:\MATLAB6p5\work\raf\dati_4feat\');
tst=load('C:\MATLAB6p5\work\raf\dati_4feat');
...
net = NEWRB(A,Tc,0.01,spread,65,5);  % addestramento
    
a = sim(net,PPNN);                   % calcolo risultati sul test
...
th = 0;
    ...
    for d = 1:1000    
        th = th + 0.01; 
        vet_soglia = soglia_rbf(a,4226,th);
        [sen(d), spe(d)] = risul_class(vet_soglia, Ts, 4226);
    end
